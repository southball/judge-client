...
const NavbarItemLeft: React.FC = ({ children }: { children: React.ReactNode }) => <>{children}</>;
const NavbarItemRight: React.FC = ({ children }: { children: React.ReactNode }) => <>{children}</>;
...
const NavbarBody = ({ toggled, children }: NavbarBodyProps) => {
  ...
  return (
    <div className={'navbar-menu' + (toggled ? ' is-active' : '')}>
      <div className="navbar-start">{leftItems}</div>
      <div className="navbar-end">{rightItems}</div>
    </div>
  );
};
...
const Nav = ({ left, right }: NavProps) => {
  ...
  const NavLink = ({ className, children, to, onClick, ...props }: NavLinkProps) => {
    ...
    return (
      <Link className={`navbar-item ${additionalClassName}`} to={to} onClick={extendedOnClick as any} {...props}>
        {children}
      </Link>
    );
  };
  ...
  return (
    <nav className="navbar is-dark is-fixed-top" style={{ boxShadow: '0 2px 0 0 #f5f5f5', zIndex: 10000 }}>
      ...
      <NavbarBody toggled={toggled}>
        <NavbarItemLeft>{left?.(NavLink)}</NavbarItemLeft>
        <NavbarItemRight>{right?.(NavLink)}</NavbarItemRight>
      </NavbarBody>
    </nav>
  );
};
